...
app.get('/api/quality', async (req, res) => {
  try {
    const qualityTrends = await pool.query(`
      SELECT 
        TO_CHAR(date, 'Mon-YY') as month,
        AVG(CASE WHEN product_produced > 0 THEN (product_rejected::float / product_produced) * 100 ELSE 0 END) as rejection_rate,
        AVG(CASE WHEN product_shipped > 0 THEN (product_returned::float / product_shipped) * 100 ELSE 0 END) as return_rate,
        SUM(COALESCE(cost_of_repair, 0) + COALESCE(cost_of_remake, 0)) as quality_costs
      FROM quality
      GROUP BY TO_CHAR(date, 'Mon-YY'), DATE_TRUNC('month', date)
      ORDER BY DATE_TRUNC('month', date) DESC
      LIMIT 12
    ...
    const costBreakdown = await pool.query(`
      SELECT 
        COALESCE(SUM(cost_of_repair), 0) as repair_costs,
        COALESCE(SUM(cost_of_remake), 0) as remake_costs,
